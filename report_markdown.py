...
elements.append(
    Paragraph(f'''
Il prototipo viene considerato un "successo" se entrambi gli obiettivi indicati nella sezione precedente ("Obiettivo del prototipo") vengono raggiunti. Maggiori dettagli sulle metriche utilizzate per validare il successo del prototipo devono essere ancora forniti.
    ''', body_style,)
)
###
...
elements.append(
    Paragraph(f'''
I prerequisiti necessari per raggiungere gli obiettivi sopra elencati sono i seguenti:
    ''', body_style,)
)
list_gen(f'''
...
Otregroup deve richiedere una dimostrazione pratica dell'utilizzo corretto del sistema di spillatura al tecnico di Pozzobon Distribuzione
...
elements.append(
    Paragraph(f'''
Le figure coinvolte in questa fase sono:
    ''', body_style,)
)
list_gen(f'''
Otregroup
...
elements.append(
    Paragraph(f'''
Lo scopo di questa fase è quello di integrare una centralina di controllo e di sviluppare il software necessario per automatizzare il ciclo di sanificazione, il quale viene effettuato post-cambio fusto premendo un pulsante nel touchscreen della centralina.
    ''', body_style,)
)
###
elements.append(Paragraph("Diagramma Flusso", h3))
...
il ciclo termina dopo la procedura di sanificazione e la spillatrice ritorna ad erogare la bibita al posto dell'acqua ozonizzata
'''.strip().split('\n'))
###
elements.append(Paragraph("Prerequisiti", h3))
elements.append(
...
list_gen(f'''
Otregroup deve ottenere un sensore di flusso (flussostato) idoneo all'applicazione e interfacciabile alla centralina di controllo (chiedere supporto a Pozzobon Distribuzione)
Otregroup deve ottenere un sensore di contatto (micro switch) o sensore di pressione (pressostato) idoneo all'applicazione e interfacciabile alla centralina di controllo (chiedere supporto a Pozzobon Distribuzione)
'''.strip().split('\n'))
###
elements.append(Paragraph("Figure Coinvolte", h3))
elements.append(
